...
class FeishuClient:
    ...
    def _tenant_token(self) -> str:
        ...
        if result.get("code") != 0:
            raise RuntimeError(f"椋炰功閴存潈澶辫触锛歿result.get('msg', result.get('code'))}")
    ...
    def send_text(self, receive_id: str, text: str, *, receive_id_type: str = "chat_id") -> str:
        ...
        if result.get("code") != 0:
            raise RuntimeError(f"椋炰功鍙戦€佸け璐ワ細{result.get('msg', result.get('code'))}")
    ...
    def list_messages(self, container_id: str, *, page_size: int = 50) -> list[dict]:
        ...
        if result.get("code") != 0:
            raise RuntimeError(f"椋炰功璇诲彇娑堟伅澶辫触锛歿result.get('msg', result.get('code'))}")
        messages = []
